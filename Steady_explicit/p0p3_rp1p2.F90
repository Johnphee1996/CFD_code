...
do itau = 1, ntime
    ...
    do ie = 1, nelem
        ...
        if (ie .eq. 1) then
            !
            p2dot(ie) = 0.4d0*((unkno(2,2) - unkno(2,1))/dx**2 + &
                        & (2.d0*unkno(3,2) - 3.d0*unkno(3,1))/dx)
            !
        elseif (ie .eq. nelem) then
            !
            p2dot(ie) = 0.4d0*((unkno(2,nelem-1) - unkno(2,nelem))/dx**2 + &
                        & (3.d0*unkno(3,nelem) - 2.d0*unkno(3,nelem-1))/dx)
            !
        else
            !
            p2dot(ie) = 0.2d0*(unkno(2,ie+1) + unkno(2,ie-1) - 2.d0*unkno(2,ie))/dx**2 &
                        & + 0.4d0*(unkno(3,ie+1) - unkno(3,ie-1))/dx
            !
        end if
        !
    end do
    ...
    do ip = 1, npoin
        ...
        if (ip .eq. 1) then
            ...
            !
            pr = unkno(2,1) - unkno(3,1)*0.5d0*dx + p2dot(1)*dx**2/12.d0
            pl = pr
            !
        elseif (ip .eq. npoin) then
            ...
            ur = 0.d0
            !
            pl = unkno(2,nelem) + unkno(3,nelem)*0.5d0*dx + p2dot(nelem)*dx**2/12.d0
            pr = pl
            !
        else
            !
            ...
            !
            pl = unkno(2,iel) + unkno(3,iel)*0.5d0*dx + p2dot(iel)*dx**2/12.d0
            pr = unkno(2,ier) - unkno(3,ier)*0.5d0*dx + p2dot(ier)*dx**2/12.d0
            !
        end if
        !
        ua = 0.5d0*(ul + ur)
    ...
    do ie = 1, nelem
        ...
        do i = 1, ngauss
            ...
            b4 = 1.d0/6.d0*(xg - xc(ie))**3 - dx**2/24.d0*(xg - xc(ie))
            !
            !source term
            !
            rhsel(1,ie) = rhsel(1,ie) + 0.5d0*dx*wg(i)*nu*pi*pi*sin(pi*xg)
            rhsel(2,ie) = rhsel(2,ie) + 0.5d0*dx*wg(i)*b2*nu*pi*pi*sin(pi*xg) - &
                          & 0.5d0*dx*wg(i)*(unkno(2,ie) + unkno(3,ie)*b2 + p2dot(ie)*b3)/Tr
            rhsel(3,ie) = rhsel(3,ie) + 0.5d0*dx*wg(i)*b3*nu*pi*pi*sin(pi*xg) - &
                          & 0.5d0*dx*wg(i)*b2*(unkno(2,ie) + unkno(3,ie)*b2 + p2dot(ie)*b3)/Tr
            !
            !dCT/dx*F domain integral
            !
            rhsel(2,ie) = rhsel(2,ie) - 0.5d0*dx*wg(i)*nu*(unkno(2,ie) + unkno(3,ie)*b2 + p2dot(ie)*b3)
            rhsel(3,ie) = rhsel(3,ie) - 0.5d0*dx*wg(i)*nu*b2*(unkno(2,ie) + unkno(3,ie)*b2 + p2dot(ie)*b3) &
                          & - 0.5d0*dx*wg(i)*(unkno(1,ie) + unkno(2,ie)*b2 + unkno(3,ie)*b3 + p2dot(ie)*b4)/Tr
            !
        end do
        !
    end do
...
do ie = 1, nelem
    ...
    do i = 1, ngauss
        ...
        b3 = 0.5d0*(xg - xc(ie))**2 - dx**2/24.d0
        b4 = 1.d0/6.d0*(xg - xc(ie))**3 - dx**2/24.d0*(xg - xc(ie))
        l2_u = l2_u + 0.5d0*dx*wg(i)*(unkno(1,ie) + unkno(2,ie)*b2 + unkno(3,ie)*b3 +p2dot(ie)*b4 - ue)**2
        l2_p = l2_p + 0.5d0*dx*wg(i)*(unkno(2,ie) + unkno(3,ie)*b2 + p2dot(ie)*b3 - pe)**2
        !
    end do
    !
end do
...
close(1)
!
open(2,access='append',file='order5.dat')
    !
    write(*,*) 'L2_error'
    !
