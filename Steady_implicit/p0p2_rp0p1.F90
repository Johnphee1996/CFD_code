...
do itau = 1, ntime
    ...
    do ie = 1, nelem
        ...
        if (ie .eq. 1) then
            !
            pdot(ie) = (unkno(2,2)-unkno(2,1))/2.d0/dx
                    !144.d0*unkno(2,2)/181.d0/dx - 210.d0*unkno(2,1)/181.d0/dx + &
                    !72.d0*unkno(1,2)/181.d0/dx**2 - 84.d0*unkno(1,1)/181.d0/dx**2
            !
        elseif (ie .eq. nelem) then
            !
            pdot(ie) = (unkno(2,nelem)-unkno(2,nelem-1))/2.d0/dx
                    !210.d0*unkno(2,nelem)/181.d0/dx - 144.d0*unkno(2,nelem-1)/181.d0/dx - &
                    !84.d0*unkno(1,nelem)/181.d0/dx**2 + 72.d0*unkno(1,nelem-1)/181.d0/dx**2   
            !
        else 
            !
            pdot(ie) = (unkno(2,ie+1)-unkno(2,ie-1))/2.d0/dx
                    !2.d0*(unkno(2,ie+1) - unkno(2,ie-1))/5.d0/dx + &
                    !(unkno(1,ie+1) + unkno(1,ie-1) - 2.d0*unkno(1,ie))/5.d0/dx**2
            !
        end if
        !
    end do
    ...
    !source term
        ...
        rhsel(2,ie) = rhsel(2,ie) + 0.5d0*dx*wg(i)*(xg-xc(ie))*nu*pi*pi*sin(pi*xg)
        !
        end do
        !
...
do ie = 1, nelem
    ...
    do i = 1, ngauss
        ...
        ue = sin(pi*xg/L)
        pe = pi/L*cos(pi*xg/L)
        l2_u = l2_u + 0.5d0*dx*wg(i)*(unkno(1,ie) + unkno(2,ie)*(xg-xc(ie)) +  &
                pdot(ie)*(0.5d0*(xg-xc(ie))**2-dx**2/24.d0) - ue)**2
        l2_p = l2_p + 0.5d0*dx*wg(i)*(unkno(2,ie) + pdot(ie)*(xg-xc(ie)) - pe)**2
        !
        !
    end do
    !
...
l2_p = sqrt(l2_p)
!
open(2,access='append',file='orderste3.dat')
write(*,*) 'L2-error'
    ! 
    !
